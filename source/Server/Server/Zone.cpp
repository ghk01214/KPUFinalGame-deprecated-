...
{
	objects[id]->GetMyObject()->Move(direction);

	//std::cout << id << ": " << objects[id]->GetMyObject()->GetX() << ", " << objects[id]->GetMyObject()->GetZ() << std::endl;

	// 움직인 플레이어에게 우선 이동 패킷 전송
	objects[id]->SendMoveObjectPacket(id, objects[id]->GetMyObject());
