...
void CObjectsShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	...
	CCubeMeshTextured* pCubeMesh = new CCubeMeshTextured(pd3dDevice, pd3dCommandList, 120.0f, 120.0f, 120.0f);  // 큐브 생성 (큐브 크기)
...
void CObjectsShader::Move()
{
	/*for (int i = 0; i < m_nObjects; i++)
	{
		if (!garo)
		{
			m_ppObjects[i]->MoveForward(1.0f);
		}
		else
		{
			m_ppObjects[i]->MoveStrafe(1.0f);
		}
	}*/
}

void CObjectsShader::OverCheck()
{
	for (int i = 0; i < m_nObjects; i++)
	{
		if (m_ppObjects[i]->GetPosition().x < 0 || m_ppObjects[i]->GetPosition().x>2000)
		{
			m_ppObjects[i]->SetPosition(1000.0f, 400.0f, 0.0f);
			garo = false;
		}
		if (m_ppObjects[i]->GetPosition().z < 0 || m_ppObjects[i]->GetPosition().z>2000)
		{
			m_ppObjects[i]->SetPosition(0.0f, 400.0f, 1000.0f);
			garo = true;
		}
	}
}
...
void CBillboardObjectsShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	...
	CRawFormatImage* pRawFormatImage = new CRawFormatImage(L"Image/Copy.raw", 257, 257, true);
